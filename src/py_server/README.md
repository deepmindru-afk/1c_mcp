...
|---|---|---|
| Папка, доступная обоим | `D:\tmp\exch` или `\\server\share\exch` | путь к той же папке (может выглядеть иначе, например `\\хост\tmp\exch`) |
...
Для `MCP_TRANSPORT=httppoll` адрес на форме — `http://localhost:9090` (адрес HTTP-листенера прокси, каким его видит машина 1С, см. ниже).
...
Признак работающей связки: файл запроса `*.json` исчезает из `in/`, ответ появляется в `out/` и тут же забирается прокси; ошибки агент показывает в поле состояния на форме. Настройки прокси (к интервалу агента отношения не имеют): `MCP_FILE_TIMEOUT` — сколько ждать ответ 1С (30 с), `MCP_FILE_POLL_INTERVAL` — как часто проверять появление файла ответа (0.2 с для папки, 1 с для FTP).

### httppoll — обратный HTTP

Работает только в stdio-режиме прокси (режим подключения клиентов, см. «Режимы работы»). Прокси поднимает HTTP-листенер (по умолчанию `127.0.0.1:9090`), агент сам обращается к нему за запросами — на машине с 1С входящие порты не нужны. Транспорту достаточно проброса одного TCP-порта (например, туннель NoMachine).
