...
## Транспорты до 1С без веб-сервера (экспериментальные)

Помимо основного пути — HTTP-сервиса 1С — прокси умеет два транспорта, не требующих веб-сервера и публикации базы. **Не для промышленной эксплуатации**: только для тестирования и для тех, кто использует MCP при разработке в 1С. Выбор — переменной `MCP_TRANSPORT` (по умолчанию `http`).

На стороне 1С оба транспорта обслуживает агент: откройте в базе обработку «Управление MCP-сервером», вкладка «MCP без веб-сервера (тест)», укажите адрес и нажмите «Старт». `MCP_ONEC_URL`, логин и пароль в этих режимах не нужны — запросы выполняются от имени пользователя, запустившего агент.

### file — обмен через папку

Прокси кладёт запросы в `<папка>/in/`, агент 1С пишет ответы в `<папка>/out/`.
...
В адресе агента — та же папка (локальный путь или `ftp://пользователь:пароль@хост/путь`). Дополнительно: `MCP_FILE_TIMEOUT` (30 с), `MCP_FILE_POLL_INTERVAL` (0.2 с).

### httppoll — обратный HTTP: агент опрашивает прокси

Прокси поднимает мини-листенер (по умолчанию `127.0.0.1:9090`), агент 1С сам забирает запросы по HTTP и возвращает ответы. TCP-соединение всегда направлено «агент → прокси», поэтому на машине с 1С не нужны ни входящие порты, ни веб-сервер — транспорт подходит для изолированных контуров, куда с локальной машины есть только проброс TCP-порта (например, туннель NoMachine).
...
В адресе агента — `http://localhost:9090`. Работает только в stdio-режиме прокси. Дополнительно: `MCP_HTTPPOLL_HOST` (127.0.0.1), `MCP_HTTPPOLL_TIMEOUT` (60 с). Диагностика листенера: `curl http://127.0.0.1:9090/health`.
