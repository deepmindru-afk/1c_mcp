...
| FTP-сервер на третьей машине — общей папки нет, прямой сети между прокси и 1С тоже | `file` | тот же `ftp://…`, что и в `MCP_FILE_EXCHANGE_DIR` прокси |
...
Прокси пишет файл запроса в `<папка>/in/` и ждёт файл ответа в `<папка>/out/`. Агент по таймеру забирает запросы из `in/` и кладёт ответы в `out/`. В `MCP_FILE_EXCHANGE_DIR` указывается либо локальный/сетевой путь, либо адрес папки на FTP-сервере — `ftp://пользователь:пароль@хост:порт/путь` (без шифрования, FTPS не поддерживается). Агент должен видеть ту же самую папку — напрямую или через FTP; вариант «FTP с обеих сторон» покрывает случай, когда общая точка обмена (FTP на третьей машине) — единственное, что доступно и прокси, и 1С.
...
Настройки прокси (к интервалу агента отношения не имеют): `MCP_FILE_TIMEOUT` — сколько ждать ответ 1С (30 с), `MCP_FILE_POLL_INTERVAL` — как часто проверять появление файла ответа (0.2 с для папки, 1 с для FTP).
